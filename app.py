...
if option == "Lọc Danh Mục Thầu":
    ...
    st.dataframe(disp[["Tên hoạt chất", conc_col, "Nhóm thuốc","Tên sản phẩm","Địa bàn","Tên Khách hàng phụ trách triển khai","Tỷ trọng nhóm thầu"]], height=500)
...
# —— 2. Phân Tích Danh Mục Thầu ——
    ...
    plot_bar(dfA.groupby("Đường")["Trị giá"].sum().reset_index(), "Đường", "Trị giá", "Trị giá theo Đường dùng")
...
# —— 4. Đề Xuất Hướng Triển Khai ——
    ...
    sug = sug.sort_values("Đề xuất", ascending=False)

    st.dataframe(sug[["_act","_conc","_grp","SL_Đã làm","SL_Yêu cầu","Đề xuất","Tên Khách hàng phụ trách triển khai"]])
